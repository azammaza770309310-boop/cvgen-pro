...
class OpenAIProvider(AIProvider):
            ...
            try:
                async with httpx.AsyncClient(timeout=60) as client:
                    ...
                    if resp.status_code in (429, 500, 502, 503):
                        last_err = f"OpenAI HTTP {resp.status_code}"
                        if self.rotate_key():
                            headers["Authorization"] = f"Bearer {self.current_key}"
                            continue
                        raise AIProviderError(f"OpenAI error {resp.status_code}: {resp.text[:200]}")
                    if resp.status_code != 200:
                        raise AIProviderError(f"OpenAI HTTP {resp.status_code}: {resp.text[:300]}")
                    data = resp.json()
                ...
                if self.rotate_key():
                    headers["Authorization"] = f"Bearer {self.current_key}"
                    continue
                raise AIProviderError(f"OpenAI network error: {e}")
        raise AIProviderError(f"OpenAI failed after retries: {last_err}")
    ...
    async def parse_resume(self, text: str, lang: str = "auto") -> dict:
        ...
        if not isinstance(data, dict):
            raise AIProviderError("OpenAI did not return valid JSON")
        return data
