...
class OpenRouterProvider(AIProvider):
    ...
    async def _chat(self, system: str, user: str, *, json_mode: bool = True) -> str:
        headers = {
            ...
            "HTTP-Referer": "https://cvgen-pro.app",
            "X-Title": "CVGen Pro",
        }
        payload = {
            ...
            try:
                async with httpx.AsyncClient(timeout=60) as client:
                    ...
                    if resp.status_code in (429, 500, 502, 503):
                        last_err = f"OpenRouter HTTP {resp.status_code}"
                        if self.rotate_key():
                            headers["Authorization"] = f"Bearer {self.current_key}"
                            continue
                        raise AIProviderError(f"OpenRouter error {resp.status_code}: {resp.text[:200]}")
                    if resp.status_code != 200:
                        raise AIProviderError(f"OpenRouter HTTP {resp.status_code}: {resp.text[:300]}")
                    data = resp.json()
                ...
                if self.rotate_key():
                    headers["Authorization"] = f"Bearer {self.current_key}"
                    continue
                raise AIProviderError(f"OpenRouter network error: {e}")
        raise AIProviderError(f"OpenRouter failed after retries: {last_err}")
